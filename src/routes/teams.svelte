<main class="p-5 rounded-md m-auto w-96 my-10 text-center text-indigo-600 border border-indigo-600">
	<h1 class="font-title text-4xl md:text-6xl uppercase megawide">teams</h1>
</main>

<div class="flex justify-center">
	<a href="/teams/id">
		<div class="rounded-lg shadow-lg bg-white max-w-sm">
			<img
				class="rounded-t-lg"
				src="https://mdbootstrap.com/img/new/standard/nature/184.jpg"
				alt="Logo"
			/>
			<div class="p-6">
				<h5 class="text-gray-900 text-xl font-medium mb-2">1ºC</h5>
				<p class="text-gray-700 text-base mb-4">Este equipo se centra en la clase de 1ºC ESO.</p>
			</div>
		</div>
	</a>
</div>
